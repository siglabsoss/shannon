...
#ifndef __POP_SINK_GPU_HPP_
#define __POP_SINK_GPU_HPP_

// include base popsink for usage of buffer_read_pointer
...
template<typename> class PopSourceGpu;
...
class PopSinkGpu : public PopObject,
...
protected:
     ...
     */
    PopSinkGpu(const char* name, size_t nInBuf = 0) : PopObject(name), m_reqBufSize(nInBuf),
...
public:
    ...
    void start_thread()
    {
        ...
        {
            m_mutex.lock();
            m_pThread = new boost::thread(boost::bind(&PopSinkGpu::run, this));
            m_mutex.lock();
        }
    }
...
private:
    ...
    // friend classes
    template <typename> friend class PopSourceGpu;

protected:
    // pointer to the source that feeds into us
    PopSourceGpu<IN_TYPE>* m_rgSource;
};
